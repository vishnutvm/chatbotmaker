        ...
        <div className="rounded-xl border border-border bg-surface p-6">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            {[
              ...
              <div key={p.name} className={`rounded-lg border p-5 ${p.current ? "border-primary ring-1 ring-primary" : "border-border"}`}>
                <div className="text-sm font-semibold text-foreground">{p.name}</div>
                <div className="mt-1 text-2xl font-semibold tracking-tight text-foreground">{p.price}<span className="text-sm font-normal text-muted-foreground"> {p.price.startsWith("$") ? "/ month" : ""}</span></div>
                <ul className="mt-4 space-y-1.5 text-sm text-foreground">
                  {p.features.map((f) => <li key={f} className="flex items-center gap-2"><Check className="h-3.5 w-3.5 text-success" /> {f}</li>)}
                </ul>
                {!p.current && <Button className="mt-5 w-full" variant={p.name === "Scale" ? "outline" : "default"}>{p.name === "Scale" ? "Contact sales" : "Upgrade"}</Button>}
              </div>
            ))}
          </div>
        </div>

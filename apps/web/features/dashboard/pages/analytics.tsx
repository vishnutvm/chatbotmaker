    ...
    <>
      ...
      <div className="mx-auto max-w-[1240px] px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
          ...
          <MetricCard label="Resolution rate" value="78%" delta={{ value: "+2pt", direction: "up" }} />
          ...
          <div className="mt-4 h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={analyticsSeries}>
                ...
                <Area type="monotone" dataKey="conversations" stroke="var(--color-primary)" strokeWidth={2} fill="url(#ga2)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            <div className="mt-4 divide-y divide-border">
              {assistants.slice(0, 4).map((a) => (
                <div key={a.id} className="flex items-center justify-between py-3 text-sm">
                  ...
                  <span className="text-muted-foreground">{a.conversations.toLocaleString()} convos</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="mt-4 h-[220px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={topTopics} layout="vertical" margin={{ left: 20 }}>
                  ...
                  <YAxis type="category" dataKey="topic" tickLine={false} axisLine={false} tick={{ fontSize: 12, fill: "var(--color-foreground)" }} width={140} />
        ...
        <div className="rounded-xl border border-border bg-surface p-5">
          ...
          <p className="text-xs text-muted-foreground">Questions your assistants couldn't answer confidently.</p>
          <ul className="mt-3 divide-y divide-border">
            {unansweredQuestions.map((q, i) => (
              <li key={i} className="flex items-center justify-between py-3 text-sm">
                <span className="text-foreground">{q}</span>
                <button className="text-xs font-medium text-primary hover:text-primary-hover">Add knowledge</button>
              </li>
            ))}
          </ul>
        </div>

...
export default function ConvoDetail() {
  ...
  return (
    <>
      <TopHeader breadcrumb={<div className="flex items-center gap-1.5"><Link href="/dashboard/conversations" className="hover:text-foreground">Conversations</Link><span>/</span><span className="text-foreground">{c.visitor}</span></div>} />
      <div className="mx-auto max-w-[1240px] px-6 py-6">
        <Button asChild variant="ghost" size="sm" className="mb-4"><Link href="/dashboard/conversations"><ChevronLeft className="mr-1 h-4 w-4" /> All conversations</Link></Button>
          ...
          <div className="rounded-xl border border-border bg-surface">
            <div className="border-b border-border px-6 py-4 flex items-center justify-between">
              <div>
                ...
                <div className="text-xs text-muted-foreground">{c.assistantName} · Web widget</div>
              </div>
              ...
              {msgs.map((m) => (
                <div key={m.id} className={m.role === "user" ? "flex justify-end" : ""}>
                  <div className={m.role === "user" ? "max-w-[75%]" : "max-w-[75%]"}>
                    <div className={m.role === "user" ? "rounded-2xl rounded-br-sm bg-primary px-4 py-2.5 text-sm text-primary-foreground" : "rounded-2xl rounded-bl-sm border border-border bg-surface-muted px-4 py-2.5 text-sm text-foreground"}>
                      {m.content}
                    </div>
                    {m.sources && (
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {m.sources.map((s) => (
                          <span key={s.url} className="inline-flex items-center gap-1 rounded-md border border-border bg-surface px-2 py-0.5 text-[11px] text-muted-foreground">

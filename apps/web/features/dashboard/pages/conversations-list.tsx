      ...
      <div className="mx-auto max-w-[1400px] px-6 py-8 space-y-6">
        ...
        <div className="grid gap-0 rounded-xl border border-border bg-surface overflow-hidden lg:grid-cols-[320px_minmax(0,1fr)_280px] min-h-[600px]">
          ...
          <div className="border-r border-border">
            ...
            <div className="divide-y divide-border max-h-[600px] overflow-y-auto">
              {conversations.map((c, i) => (
                ...
                >
                  <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-surface-muted text-[11px] font-semibold text-foreground">{c.visitorInitials}</div>
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1"><StatusBadge tone={c.status === "resolved" ? "success" : c.status === "open" ? "info" : "warning"}>{c.status}</StatusBadge></div>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="hidden lg:flex flex-col">
            ...
            <div className="border-t border-border px-6 py-3 text-center text-xs text-muted-foreground">Read-only preview · <Link href={`/dashboard/conversations/${conversations[0]?.id ?? ''}`} className="text-primary hover:underline">Open conversation</Link></div>
          </div>

          {/* Details */}
          <div className="hidden lg:block border-l border-border p-5 space-y-4">
            <div>
              <div className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Assistant</div>
            ...
            <div>
              ...
              <div className="mt-1"><StatusBadge tone="info">open</StatusBadge></div>
            </div>
            <div>
              <div className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Channel</div>
              <div className="mt-1 text-sm text-foreground">Web widget</div>
            </div>
            <div>
              <div className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Messages</div>
              <div className="mt-1 text-sm text-foreground">4</div>
            </div>
            <div className="border-t border-border pt-4">
              <div className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Visitor</div>
...
function Bubble({ role, children }: { role: "user" | "assistant"; children: React.ReactNode }) {
  return (
    <div className={role === "user" ? "flex justify-end" : ""}>
      <div className={role === "user" ? "max-w-[70%] rounded-2xl rounded-br-sm bg-primary px-3.5 py-2 text-sm text-primary-foreground" : "max-w-[70%] rounded-2xl rounded-bl-sm border border-border bg-surface-muted px-3.5 py-2 text-sm text-foreground"}>
        {children}
      </div>
    </div>
  );

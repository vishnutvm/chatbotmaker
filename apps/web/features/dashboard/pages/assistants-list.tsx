...
export default function AssistantsList() {
  ...
  const filtered = useMemo(() => {
    ...
    // "updated" preserves insertion order (newest first via addAssistant)
    return list;
      ...
      <div className="mx-auto max-w-[1240px] px-4 sm:px-6 py-8 space-y-6">
        ...
        {filtered.length === 0 ? (
          <EmptyState
            ...
            action={
              assistants.length === 0 ? (
                <Button asChild><Link href="/dashboard/assistants/new/create"><Plus className="mr-1.5 h-4 w-4" /> Create assistant</Link></Button>
              ) : (
                ...
                <tbody className="divide-y divide-border">
                  {filtered.map((a) => (
                    ...
                    >
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <div className="truncate text-sm font-medium text-foreground group-hover:text-primary">{a.name}</div>
                      ...
                      <td className="hidden lg:table-cell px-5 py-3.5 text-sm text-foreground">{a.knowledgeSources} sources</td>
                      <td className="px-5 py-3.5 text-sm text-foreground">{a.conversations.toLocaleString()}</td>
                      <td className="hidden lg:table-cell px-5 py-3.5 text-sm text-muted-foreground">{a.lastUpdated}</td>
                      <td className="px-5 py-3.5" onClick={(e) => e.stopPropagation()}>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => router.push(`/dashboard/assistants/${a.id}/overview`)}>Open</DropdownMenuItem>
                            <DropdownMenuItem onClick={() => toast.success(`Duplicated ${a.name}`)}>Duplicate</DropdownMenuItem>
                            <DropdownMenuItem onClick={() => toast.info(`${a.name} paused`)}>Pause</DropdownMenuItem>
                            <DropdownMenuItem
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 flex gap-4 text-[11px] text-muted-foreground">
                        <span><b className="font-medium text-foreground">{a.conversations.toLocaleString()}</b> conversations</span>
                        <span><b className="font-medium text-foreground">{a.knowledgeSources}</b> sources</span>
                      </div>
                    </div>
                  </div>
                </Link>
